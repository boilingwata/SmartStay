...
-- Atomic payment processing: inserts a payment row + syncs invoice in one transaction.
-- Called by the process-payment Edge Function via supabase.rpc().
-- Uses TEXT parameters (not enum casts) because these columns use CHECK constraints, not Postgres enums.
...
-- Approve an existing (pending) payment and sync the invoice atomically.
