...
is 'Persist utility invoice in one transaction: invoice + invoice_items + mandatory immutable snapshot.';

update smartstay.contracts
set occupants_for_billing = 1,
    terms = coalesce(terms, '{}'::jsonb) || jsonb_build_object('occupants_for_billing', 1)
where occupants_for_billing <= 0;
...
update smartstay.services
set is_active = false
where id in (1, 2);

update smartstay.utility_policies
set code = 'SYS-DEFAULT-UTILITY-2026',
    name = 'System Utility Policy 2026',
    description = 'Production-ready system default policy for SmartStay utility billing.',
    electric_base_amount = 220000,
    water_base_amount = 50000,
    water_per_person_amount = 40000,
    electric_hot_season_multiplier = 1.15,
    location_multiplier = 1.0,
    season_months = '["04","05","06","07","08","09"]'::jsonb,
    rounding_increment = 1000,
    min_electric_floor = 120000,
    min_water_floor = 60000,
    is_active = true,
