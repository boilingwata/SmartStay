...
grant execute on function smartstay.create_policy_utility_invoice(bigint, text, date, numeric, numeric, text, jsonb, jsonb) to authenticated;

comment on function smartstay.create_policy_utility_invoice(bigint, text, date, numeric, numeric, text, jsonb, jsonb)
is 'Persist invoice utility policy-based trong 1 transaction: invoices + invoice_items + invoice_utility_snapshots.';
