...
const OwnerList = () => {
  ...
  const maskValue = (val: string, show: boolean) => {
    if (!val) return '---';
    if (show || val.length <= 8) return val;
  ...
  const handleExport = () => {
    ...
    const exportData = owners.map((owner) => ({
      'Họ tên': owner.fullName,
      'Số nhà sở hữu': owner.totalBuildings,
      ...(canViewPII
        ? {
            'Số điện thoại': owner.phone,
            'Email': owner.email,
            'Mã số thuế': owner.taxCode ?? '',
            'CCCD': owner.cccd ?? '',
          }
        : {}),
    }));
    ...
    toast.success(
      canViewPII
        ...
        : 'Đã xuất file không bao gồm dữ liệu PII'
    );
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2 p-1 bg-bg/50 rounded-xl border border-border/50">
                  <button type="button" onClick={() => setFilterActive(undefined)} className={cn('flex-1 h-9 rounded-lg text-[11px] font-black uppercase transition-all', filterActive === undefined ? 'bg-white shadow-sm text-primary' : 'text-muted hover:text-primary')}>Tất cả</button>
                  <button type="button" onClick={() => setFilterActive(true)} className={cn('flex-1 h-9 rounded-lg text-[11px] font-black uppercase transition-all', filterActive === true ? 'bg-white shadow-sm text-success' : 'text-muted hover:text-success')}>Hoạt động</button>
                  <button type="button" onClick={() => setFilterActive(false)} className={cn('flex-1 h-9 rounded-lg text-[11px] font-black uppercase transition-all', filterActive === false ? 'bg-white shadow-sm text-danger' : 'text-muted hover:text-danger')}>Đã khóa</button>
                </div>
              </div>
      ...
      {isLoading ? (
        ...
        <div className="card-container border-dashed border-primary/20 bg-white/70">
          <EmptyState
            icon={Users}
            title="Chưa có chủ sở hữu phù hợp"
            message="Không tìm thấy chủ sở hữu nào theo bộ lọc hiện tại. Hãy điều chỉnh tiêu chí tìm kiếm hoặc thêm hồ sơ mới."
            actionLabel="Thêm chủ sở hữu"
                  ...
                  <tr key={owner.id} className="group hover:bg-primary/[0.02] transition-colors">
                    <td className="px-6 py-6 font-medium">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-body font-black text-primary hover:underline cursor-pointer leading-tight" onClick={() => navigate(`/admin/owners/${owner.id}`)}>
                            {owner.fullName}
                          </p>
                      ...
                      <div className="relative group/tooltip inline-block cursor-help">
                        ...
                        <div className="absolute bottom-full left-0 mb-3 w-64 bg-slate-900 text-white rounded-2xl p-4 shadow-2xl opacity-0 group-hover/tooltip:opacity-100 transition-all pointer-events-none z-10 translate-y-2 group-hover/tooltip:translate-y-0">
                          ...
                          <ul className="space-y-3">
                            {owner.buildingsOwned.map((building, idx) => (
                              <li key={`${building.buildingId}-${idx}`} className="text-[11px] font-bold flex items-center gap-2 text-slate-300">
                                <span className="w-1.5 h-1.5 bg-primary rounded-full" /> {building.buildingName}
                              </li>
                            ))}
                          </ul>
                    ...
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-3 font-mono text-[13px] font-bold text-muted bg-bg/50 px-4 py-2 rounded-xl group-hover:bg-white transition-all group-hover:shadow-sm">
                        ...
                        {canViewPII && (
                          <button type="button" onClick={() => toggleMask(owner.id)} className="ml-2 hover:text-primary transition-colors">
                    ...
                    <td className="px-6 py-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" className="p-3 bg-white border border-border/50 rounded-2xl text-muted hover:text-primary hover:shadow-lg transition-all shadow-sm" onClick={() => navigate(`/admin/owners/${owner.id}`)} aria-label={`Xem chi tiết ${owner.fullName}`}>
                          <ExternalLink size={20} />
                        </button>
                        <button type="button" disabled title="Chưa có thao tác bổ sung" className="p-3 bg-white border border-border/50 rounded-2xl text-muted/40 transition-all shadow-sm cursor-not-allowed disabled:opacity-50" aria-label={`Chưa có thao tác bổ sung cho ${owner.fullName}`}>
      ...
      {!canViewPII && (
        <div className="p-6 bg-warning/5 border border-warning/10 rounded-3xl flex gap-4">
          ...
          <p className="text-small text-warning font-bold italic">
            ...
            Vui lòng liên hệ Admin để yêu cầu quyền `pii.view`.
          </p>
        </div>
      )}

      <OwnerModal isOpen={isModalOpen} onClose={handleCloseModal} initialData={editingOwner} onSubmit={handleSubmit} isSubmitting={createMutation.isPending} />
    </div>
  );
};
